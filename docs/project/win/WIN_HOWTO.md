...
| **CI/CD** | `.github/workflows/release.yml` | ~80 | Windows job (MSVC + Qt6), signed artifacts, apt repo publication (coexists with website on `gh-pages`). |
...
| **Packaging (Linux)** | **Unaffected** — `.deb`, AppImage, apt repo in `release.yml` (separate Linux job). |
