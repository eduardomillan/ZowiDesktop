...
// Applies one already-parsed robot message (from zowi::MessageParser) to the
// shared global state. Caller must NOT hold g_mtx.
void parseRobotMessage(const zowi::RobotMessage &msg);

// Incoming-data callback for the Bluetooth backends. Feeds the shared
// zowi::MessageParser (which owns frame reassembly) and applies each parsed
